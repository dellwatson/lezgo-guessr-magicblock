...
const ROOM_POOL_SEED = 'room-pool';
...
async function main() {
  ...
  const [roomPoolPda] = PublicKey.findProgramAddressSync(
    [Buffer.from(ROOM_POOL_SEED)],
    guessrProgramId
  );
  ...
  const ix = new TransactionInstruction({
    ...
    keys: [
      ...
      { pubkey: roomPoolPda, isSigner: false, isWritable: true },
  ...
  console.log('Room pool PDA:', roomPoolPda.toBase58());
  console.log('Mint authority PDA:', mintAuthorityPda.toBase58());
  console.log('Signature:', signature);
  writeReport(
    '03_initialize_guessr_system.log',
    `signature=${signature} lobby=${lobbyStatePda.toBase58()} rankedConfig=${rankedConfigPda.toBase58()} roomPool=${roomPoolPda.toBase58()} mintAuthority=${mintAuthorityPda.toBase58()}`
  );
}
