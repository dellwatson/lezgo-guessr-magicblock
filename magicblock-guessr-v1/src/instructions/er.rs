...
pub const DELEGATE_TARGET_ROOM_POOL: u8 = 10;
...
pub fn delegate_guessr_state_handler(
    ...
    match args.target {
        ...
        DELEGATE_TARGET_ROOM_POOL => ctx.accounts.delegate_pda(
            &ctx.accounts.payer,
            &[b"room-pool"],
            config,
        )?,
...
pub fn commit_pool_with_reward_handler(
    ctx: Context<CommitPoolWithReward>,
    mint_amount: u64,
) -> Result<()> {
    let reward_mint = ctx.accounts.ranked_config.reward_mint;
    let player = ctx.accounts.player_profile.player;
    let player_token_account = get_associated_token_address(&player, &reward_mint);
    let (mint_authority, _) =
        Pubkey::find_program_address(&[b"mint-authority"], ctx.program_id);

    let instruction_data =
        InstructionData::data(&crate::instruction::MintPoolReward { amount: mint_amount });
    let action_args = ActionArgs::new(instruction_data);
    let action_accounts = vec![
        ShortAccountMeta {
            pubkey: ctx.accounts.ranked_config.key(),
...
#[commit]
#[derive(Accounts)]
pub struct CommitPoolWithReward<'info> {
    #[account(mut)]
    pub payer: Signer<'info>,
    #[account(
        mut,
