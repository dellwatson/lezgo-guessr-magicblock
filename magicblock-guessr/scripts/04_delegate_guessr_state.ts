...
function parseRankedRoomEntry(entry: string): RankedRoomInput | null {
  const trimmed = entry.trim();
  if (!trimmed) {
    return null;
  }

  const barIndex = trimmed.lastIndexOf('|');
  const colonIndex = trimmed.lastIndexOf(':');
  const separatorIndex = barIndex !== -1 ? barIndex : colonIndex;
  if (separatorIndex === -1) {
    return null;
  }

  const challengeId = trimmed.slice(0, separatorIndex).trim();
  const roomId = trimmed.slice(separatorIndex + 1).trim();
...
function collectRankedRooms() {
  const entries = [
    ...parseCsv(process.env.DELEGATE_RANKED_ROOMS),
    ...parseCsv(process.env.DELEGATE_RANKED_CHALLENGES),
  ];

  const rooms: RankedRoomInput[] = [];
  const invalid: string[] = [];
  const seen = new Set<string>();

  for (const entry of entries) {
    const parsed = parseRankedRoomEntry(entry);
    if (!parsed) {
      invalid.push(entry);
      continue;
    }
    const key = `${parsed.challengeId}:${parsed.roomId}`;
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
  ...
  const rankedChallengeId = process.env.RANKED_CHALLENGE_ID?.trim();
  const rankedRoomId = process.env.RANKED_ROOM_ID?.trim();
  if (rankedChallengeId && rankedRoomId) {
    const key = `${rankedChallengeId}:${rankedRoomId}`;
    if (!seen.has(key)) {
      seen.add(key);
      rooms.push({ challengeId: rankedChallengeId, roomId: rankedRoomId });
    }
  } else if (rankedChallengeId || rankedRoomId) {
    console.warn(
      'RANKED_CHALLENGE_ID or RANKED_ROOM_ID is missing. Provide both to delegate ranked rooms.'
    );
  }

  if (invalid.length > 0) {
    console.warn(
      `Skipping ranked room entries without challengeId:roomId pairs: ${invalid.join(', ')}`
    );
  }
...
async function main() {
  ...
  const rankedRooms = collectRankedRooms();

  for (const player of players) {
    for (const room of rankedRooms) {
      const challengeHash = toFixedBytes32(`${room.challengeId}:${room.roomId}`);
      const [rankedRoomPda] = PublicKey.findProgramAddressSync(
        [RANKED_ROOM_SEED, player.toBytes(), challengeHash],
        programId
      );
      addSpec(specs, {
        label: `ranked-room:${player.toBase58()}:${room.challengeId}:${room.roomId}`,
